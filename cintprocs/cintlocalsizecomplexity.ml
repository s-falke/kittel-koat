...
let getE c =
  match c with
    ...
    | _ -> failwith "Internal error in Cintlocalsizecomplexity.getE"

let getS c =
  match c with
    | (SumPlusConstant _ , _) -> Big_int.unit_big_int
    | (ScaledSumPlusConstant (_, s), _) -> s
    | _ -> failwith "Internal error in Cintlocalsizecomplexity.getS"

let getConstant c =
  match c with
    | (P p, v) -> Expexp.getConstant p
    | _ -> failwith "Internal error in Cintlocalsizecomplexity.getConstant"
...
let rec computeLocalSizeComplexities cint =
  List.flatten (List.map (fun (rule, lsbs) -> List.map (fun lsb -> (rule, lsb)) lsbs) (List.map computeLocalSizeComplexitiesOne cint))
and computeLocalSizeComplexitiesOne rule =
  let cond = Pc.dropNonLinearAtoms (Comrule.getCond rule)
  and lvars = Term.getVars (Comrule.getLeft rule) in
    let lvarswithnums = annotate 0 lvars
    and rs = Comrule.getRights rule in
...
and dumpOneLSC (rule, lsb) =
  (Comrule.toString rule) ^ ":: " ^ (dumpLSC lsb)
and dumpLSC ((i, j), c_vars) =
  (string_of_int i) ^ "." ^ (string_of_int j) ^ ": " ^ (toStringLocalComplexity c_vars)
and dumpLSCDot (rule, lsb) =
  (Comrule.toDotString rule) ^ ":: " ^ (dumpLSC lsb)

let rec dumpGSCs ruleWithGSCs =
  String.concat "\n" (List.map dumpOneGSC ruleWithGSCs)
and dumpOneGSC (rule, gsb) =
  (Comrule.toString rule) ^ ":: " ^ (dumpGSC gsb)
and dumpGSC ((i, j), c) =
  (string_of_int i) ^ "." ^ (string_of_int j) ^ ": " ^ (toStringLocalComplexity c)
