...
and getNodes rva =
  let res = ref "" in
    for i = 0 to (Array.length rva - 1) do
      res := !res ^ "\n  \"" ^ (Cintlocalsizecomplexity.dumpLSCDot (snd rva.(i))) ^ "\""
    done;
    !res
and getEdges g rva =
    ...
    for i = 0 to (len - 1) do
      for j = 0 to (len - 1) do
        if G.mem_edge g (fst rva.(i)) (fst rva.(j)) then
          accu := !accu @ ["  \"" ^ (Cintlocalsizecomplexity.dumpLSCDot (snd rva.(i))) ^ "\" -> \"" ^ (Cintlocalsizecomplexity.dumpLSCDot (snd rva.(j))) ^ "\""]
      done
    done;
    String.concat "\n" !accu
    ...
    and num1 = snd (first (snd (snd rva.(i)))) in
      for j = 0 to (len - 1) do
        ...
        and dep2 = third (snd (snd rva.(j))) in
          let argterm = List.nth (Comrule.getRights r1) argnum1
          and c1 = Comrule.getCond r1
          and leftterm = Comrule.getLeft r2
          and c2 = Comrule.getCond r2 in
            if (Cintgraph.hasEdge tgraph r1 r2) && (Utils.contains dep2 num1) && (Cintgraph.connectableOne leftterm c1 c2 argterm) then
              edges.(i).(j) <- true
      done
  done
and first (x, (_, _)) =
...
and getNum rva rv =
  ...
  and tmp2 = snd rv in
    try
      ...
      for i = 0 to (Array.length rva) - 1 do
        if (Cintlocalsizecomplexity.equalLSC tmp2 (snd (snd rva.(i)))) && (Comrule.equal tmp1 (fst (snd rva.(i)))) then
          raise (Found i)
      done;
...
and getNums rva rvs =
  let res = ref [] in
    for i = 0 to (Array.length rva) - 1 do
      ...
      if (List.exists (fun rv' -> (Cintlocalsizecomplexity.equalLSC (snd rv') (snd tmp)) && (Comrule.equal (fst rv') (fst tmp))) rvs) then
        res := i::!res
    done;
    !res
and isIn listref j =
...
and getPairs rva rules =
  let res = ref [] in
    for i = 0 to (Array.length rva) - 1 do
      let entry = rva.(i) in
        if (Cint.contains rules (fst (snd entry))) then
          res := (i, fst entry)::!res
    done;
    !res
...
and addNeededEdgesOne g tgraph rva (i, (r1, lsc)) =
  ...
  and dep1 = third lsc in
    if (Cintgraph.hasEdge tgraph r1 r1) && (Utils.contains dep1 num1) then
    ...
    for j = 0 to (Array.length rva - 1) do
      ...
      and dep2 = third (snd (snd rva.(j))) in
        let argterm1 = List.nth (Comrule.getRights r1) argnum1
        and leftterm1 = Comrule.getLeft r1
        and c1 = Comrule.getCond r1
        and argterm2 = List.nth (Comrule.getRights r2) argnum2
        and leftterm2 = Comrule.getLeft r2
        and c2 = Comrule.getCond r2 in
          if (Cintgraph.hasEdge tgraph r1 r2) && (Utils.contains dep2 num1) && (Cintgraph.connectableOne leftterm2 c1 c2 argterm1) then
            res := G.add_edge !res i (fst rva.(j));
          if (Cintgraph.hasEdge tgraph r2 r1) && (Utils.contains dep1 num2) && (Cintgraph.connectableOne leftterm1 c2 c1 argterm2) then
...
and getComplementPairs rva rules =
  let res = ref [] in
    for i = 0 to (Array.length rva) - 1 do
      let entry = rva.(i) in
        if not (Cint.contains rules (fst (snd entry))) then
          res := (i, fst entry)::!res
    done;
    !res
...
and equalSccs scc1 scc2 =
  scc1 == scc2 || ((List.length scc1) = (List.length scc2) && (List.for_all2 (fun (r1, lsc1) (r2, lsc2) -> (Cintlocalsizecomplexity.equalLSC lsc1 lsc2) && (Comrule.equal r1 r2)) scc1 scc2))
...
and dumpScc (scc, trivial) =
  "{ " ^ (String.concat "; " (List.map Cintlocalsizecomplexity.dumpLSCDot scc)) ^ " }" ^ (if trivial then " *" else "")
          ...
          let preds = getPreds rvgraph scc in
            if preds = [] then
              ...
              let sortedPreds = sortPreds preds (snd lsc) in
                ...
                and outer = Cintlocalsizecomplexity.toSmallestComplexity lsc vars
                and activeVars = List.map (List.nth vars) (snd lsc) in
                  let plugged = List.map (plug_them_in outer activeVars) plugins in
                    Cintlocalsizecomplexity.listMax (List.map (fun x -> c2lsc x vars) plugged) vars
    else
...
and insertInRet deps ret p =
  match deps with
    | [] -> failwith "internal error in Cintrvgraph.insertInRet"
...
and getPlugins sortedPreds vars accu =
  match sortedPreds with
    | [] -> []
    | [x] -> List.map (fun y -> [Cintlocalsizecomplexity.toSmallestComplexity (getGSCForOne accu y) vars]) x
    | x::rest -> let tmp = getPlugins rest vars accu in
                   List.flatten (List.map (fun y -> insertIntoAll (Cintlocalsizecomplexity.toSmallestComplexity (getGSCForOne accu y) vars) tmp) x)
...
and c2lsc c vars =
  Cintlocalsizecomplexity.complexity2localcomplexity c vars
...
and gscForNonTrivialScc scc condensed rc rvgraph vars accu =
  if List.exists isTooBig scc then
    (Cintlocalsizecomplexity.Unknown, [])
  else
...
and isTooBig ruleWithLSC =
  let lsc = getLSC (ruleWithLSC) in
    match lsc with
      | (Cintlocalsizecomplexity.Max _, _) -> false
      | (Cintlocalsizecomplexity.MaxPlusConstant _, _) -> false
      | (Cintlocalsizecomplexity.SumPlusConstant _, _) -> false
      | (Cintlocalsizecomplexity.ScaledSumPlusConstant _, _) -> false
      | (Cintlocalsizecomplexity.P p, _) -> not (Cintlocalsizecomplexity.isConstant lsc)
      | (Cintlocalsizecomplexity.Unknown, _) -> true
and isPossiblyScaledSumPlusConstant ruleWithLSC =
  match (getLSC ruleWithLSC) with
    | (Cintlocalsizecomplexity.SumPlusConstant _, _) -> true
    | (Cintlocalsizecomplexity.ScaledSumPlusConstant _, _) -> true
    | _ -> false
and isMaxPlusConstant ruleWithLSC =
  match (getLSC ruleWithLSC) with
    | (Cintlocalsizecomplexity.MaxPlusConstant _, _) -> true
    | _ -> false
and isMax ruleWithLSC =
  match (getLSC ruleWithLSC) with
    | (Cintlocalsizecomplexity.Max _, _) -> true
    | _ -> false
...
and inScc scc rv =
  (List.exists (fun rv' -> (Cintlocalsizecomplexity.equalLSC (snd rv) (snd rv')) && ((fst rv) == (fst rv'))) scc) ||
  (List.exists (fun rv' -> (Cintlocalsizecomplexity.equalLSC (snd rv) (snd rv')) && (Comrule.equal (fst rv) (fst rv'))) scc)
and isNone opt =
  match opt with
    | None -> true
    | Some _ -> false
and getAsPol ruleWithLSC =
  let lsc = getLSC ruleWithLSC in
    (Cintlocalsizecomplexity.P (Expexp.fromConstant (Cintlocalsizecomplexity.getE lsc)), [])
and getGSC accu scc =
  match accu with
    | [] -> failwith "Not found"
